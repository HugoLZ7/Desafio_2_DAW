...
document
    .getElementById("registrarPrestamo")
    .addEventListener("click", function () {
        ...
        if (!isNaN(montoPrestado) && fechaInicio) {
            ...
            if (clientes[clienteIndex].tienePrestamo) {
                alert(
                    "Este cliente ya tiene un préstamo activo. Debe pagarlo antes de solicitar otro."
                );
                return;
            }
            ...
            clientes[clienteIndex].fechaPrestamo = fechaInicio;
            ...
            clientes[clienteIndex].fechaPrestamo = fechaInicio; //guardar la fecha seleccionad
...
function marcarComoPagado(index) {
    ...
    alert(
        "El préstamo ha sido marcado como pagado. Ahora puede solicitar un nuevo préstamo."
    );

    // Actualizar la tabla
    actualizarTabla();
}
//FUNCION PARA OBTENER LA FECHA SELECCIONDA EN CALENDARIO
document.getElementById("fechaFiltro").addEventListener("change", function () {
    const fechaSeleccionada = this.value;
    const prestamosFiltrados = clientes.filter(cliente =>
      cliente.fechaPrestamo === fechaSeleccionada
    );
  
    const contenedor = document.getElementById("prestamosPorFecha");
  
    if (prestamosFiltrados.length === 0) {
      contenedor.innerHTML = '<p class="text-danger">No hay préstamos registrados en esa fecha.</p>';
    } else {
      let html = '<ul class="list-group">';
      prestamosFiltrados.forEach(p => {
        html += `<li class="list-group-item">
          <strong>Cliente:</strong> ${p.nombre}<br>
          <strong>Monto:</strong> $${p.montoPrestado.toFixed(2)}<br>
          <strong>Fecha:</strong> ${p.fechaPrestamo}
        </li>`;
      });
      html += '</ul>';
